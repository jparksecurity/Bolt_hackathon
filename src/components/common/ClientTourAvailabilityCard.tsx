...
import { Calendar, Clock, User, Mail, MessageSquare, Trash2, Edit3, X, Save } from "lucide-react";
...
export const ClientTourAvailabilityCard: React.FC<ClientTourAvailabilityCardProps> = ({
  projectId,
}) => {
  const { user } = useUser();
  ...
  const groupAvailabilitiesByClient = () => {
    ...
    availabilities.forEach(availability => {
      const key = availability.client_email || availability.client_name || 'Anonymous';
      if (!grouped[key]) {
        grouped[key] = [];
      }
    ...
    <div className="dashboard-card p-6">
      <div className="flex items-center space-x-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-600 text-sm">
            {availabilities.length} availability request{availabilities.length !== 1 ? 's' : ''} from clients
          </p>
        </div>
      </div>

      <div className="space-y-6">
        {Object.entries(groupedAvailabilities).map(([clientKey, clientAvailabilities]) => {
          const firstAvailability = clientAvailabilities[0];
          const hasContactInfo = firstAvailability.client_name || firstAvailability.client_email;
          
          return (
            <div key={clientKey} className="border border-gray-200 rounded-lg p-4">
              {/* Client Header */}
              <div className="flex items-center space-x-3 mb-4 pb-3 border-b border-gray-100">
                ...
                <div className="flex-1">
                  {hasContactInfo ? (
                    ...
                    <h4 className="font-medium text-gray-900">Anonymous Client</h4>
                  )}
                </div>
                <span className="text-xs text-gray-500">
                  {clientAvailabilities.length} time{clientAvailabilities.length !== 1 ? 's' : ''}
                </span>
              </div>
              ...
              <div className="space-y-3">
                {clientAvailabilities.map((availability) => {
                  const { dateStr, timeStr } = formatDateTime(availability.proposed_datetime);
                  const isEditing = editingId === availability.id;

                  return (
                    <div key={availability.id} className="flex items-center justify-between bg-gray-50 rounded-lg p-3">
                      <div className="flex-1">
                        {isEditing ? (
                ...
                <div className="mt-4 pt-3 border-t border-gray-100">
                  <div className="flex items-start space-x-2">
                    ...
                    <div>
                      ...
                      <p className="text-sm text-gray-600">{firstAvailability.notes}</p>
                    </div>
                  </div>
                </div>
              )}
